      ...
      for new_data in self.session:
        if new_data:
          ...
          try:
            ...
            vs  = self.fix.TPV['climb']*196.85
            spd = self.fix.TPV["speed"]*3.6
            hdg = self.fix.TPV["track"] if spd > 2 else 0
            self.alt = self.fix.TPV["alt"]*3.28084
  ...
  def run(self):
    while not stopFlag:
      if self.imu.IMURead():
        ...
        pressure = data["pressure"] / math.pow(1.0 - (alt/44330.0), 5.255) if type(alt) is float else 0
